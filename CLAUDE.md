...
- **Placement (Qwirkle-strict on colour):** every orthogonally abutting segment-pair — between laid tiles and existing tiles, and between laid tiles within the same turn — must be the **same colour**. After turn 1, **each separately-placed run** of hedges must touch ≥1 existing hedge — you may lay several *unrelated* hedges in one turn (they need not connect to each other), but none may float free of the network. (The old "all of a turn's hedges form one connected group" rule was dropped — see `validateMove` in `placement.ts`.)
